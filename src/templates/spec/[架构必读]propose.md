...
1. **架构模式第一刀（MVVM / MVI，由用户选择）**
   - 一个界面级 change 首先**沿选定的架构模式**切成两个可并行的大任务：**View 层任务** 与 **状态 / 逻辑层任务**。这一刀让「视觉还原」与「逻辑实现」解耦，可由不同专长的人并行推进。
   - **模式由用户在 propose 时选择**（见 `/cwf:propose` skill 用 AskUserQuestion 询问）；**不得擅自默认**，选定结果写进 `design.md`，apply 阶段按此实现。
     - **MVVM**：**UI 任务**（View）＋ **ViewModel 任务**（状态 / 逻辑 / 数据）。契约载体为 ViewModel 暴露的可观察数据 / 状态。
     - **MVI**：**UI 任务**（View：渲染唯一不可变 State、向上发出 Intent）＋ **State/Reducer 任务**（Intent → State 的单向数据流、业务逻辑 / 数据）。契约载体为 State 数据类与 Intent 事件集。
   - 无论哪种模式，第一刀切出的都是「View 层」与「状态/逻辑层」两块，后续拆解规则（模块控件、拓扑依赖）通用。

2. **UI 再按模块控件拆**
   - UI 大任务继续拆为独立、可复用、边界清晰的**模块控件**（导航栏、列表项、底部栏、空状态、弹窗……）。
   - 每个控件是一个可独立开发、可独立认领的子任务。
   - **拆出的每个控件必须先查 `[UI控件必读]ui-components.md`（路由入口）**：先按其「大类路由表」定位大类 → 读 `ui-components/` 下对应大类明细。命中「标准情况」的，按其「指定实现」写进 design.md / tasks.md（并注明参考实现）；命中「需人工裁定的情况」或未命中任何大类的，不要擅自定方案，标注出来交用户裁定后再写。

3. **拓扑依赖梳理**
   - 每个任务在 `tasks.md` 中按依赖顺序排列，显式体现前置关系。
   - 契约先行（MVVM：ViewModel 数据契约；MVI：State/Intent 契约）→ 控件并行 → 界面组装 → 联调。
...
| `design.md` | 实现方案、**选定的架构模式（MVVM / MVI）及其分层**、UI 框架边界、关键决策 | proposal |
...
- 架构模式（MVVM / MVI）已由用户选定并记入 `design.md`，其两大任务（View 层 / 状态·逻辑层）已切分，UI 任务已拆到模块控件粒度。
