...
CustomWorkFlow 是面向 Android/前端 工程的 AI-native 开发工具，将需求规划、设计稿读取、
...
CustomWorkFlow registers `cwf` (with `mercury` as a compatibility alias),
rather than `openspec`, so it does not overwrite an existing OpenSpec
installation. Changes are created under
`<current-project>/cwfspec/changes/`, based on the command's working
directory, and never inside the installed CustomWorkFlow package.

The workflow starts with `cwf-preflight`: it creates the change, checks
whether the PRD's feature already exists in the codebase, and records unclear
state, boundary, and compatibility items in `preflight.md`. `cwf-propose`
then verifies that change exists before creating the remaining artifacts.

To install the bundled skills into a project:

```bash
customworkflow install /path/to/your-project
```
...
```bash
customworkflow install /path/to/your-project --no-interactive
```

The command writes the bundled skills to `.claude/skills/` and
`.codex/skills/`. `customworkflow init` remains available as a compatibility
alias for `install`.

During initialization, if the target project already contains
`openspec/specs/`, its project specs are copied recursively into
`.customworkflow/spec/` with their directory structure preserved. Running init again
