---
name: falla-propose
description: 在 falla-preflight 已创建的 change 目录中，一次性生成 proposal、specs、design 和 tasks。适用于用户已完成 preflight，并希望基于 PRD 与 preflight.md 创建完整变更提案时。
...
0. **【强制】确保 spec 约束已在上下文中（不得跳过）**
   ...
   - 本阶段必读：`.falla/spec/[架构必读]propose.md`（回退：`spec/[架构必读]propose.md`）
...
1. **【强制】确认 change 目录存在**
   ...
   - 若命令返回 change 不存在或错误，停止并提示用户先运行 `falla-preflight`。不得在 propose 中运行 `falla new change`。
...
4. **按顺序创建 artifact，直至可 apply**
   ...
   a. **对每个状态为 `ready` 的 artifact：**
      - 获取指引：
        ```bash
        falla instructions <artifact-id> --change "<name>" --json
        ```
   ...
   b. **持续处理，直至所有 `applyRequires` artifact 完成：**
      - 每创建一个 artifact 后重新运行 `falla status --change "<name>" --json`。
...
- 提示运行 `falla-apply-change`

**Artifact 创建指南**

- 对每种 artifact 遵循 `falla instructions` 返回的 `instruction`。
...
- propose 只处理已有 change；不得运行 `falla new change`。
