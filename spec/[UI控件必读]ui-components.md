...
- **官方标准用法**（依据 https://github.com/hackware1993/MagicIndicator 官方文档）：
  ...
  2. **布局**：`MagicIndicator` 与 ViewPager 上下摆放：
     ```xml
  ...
  4. **联动 ViewPager**：`ViewPagerHelper.bind(magicIndicator, viewPager)`
     - **ViewPager2**：官方 `ViewPagerHelper` 面向经典 `ViewPager`。ViewPager2 场景需用适配 ViewPager2 的 helper（很多项目自带 `ViewPager2Helper.bind(magicIndicator, viewPager2)`），接入项目时按其现有封装使用。
  5. **无 ViewPager / Fragment 切换**：用 `FragmentContainerHelper`，切换时手动调用 `handlePageSelected(index)`：
     ```kotlin
...
- **关键 API 一览**：
  ...
  | `ViewPagerHelper` | `bind(indicator, viewPager)` 与 ViewPager 联动 |
  | `FragmentContainerHelper` | 非 ViewPager 场景手动联动 |

- **标准情况（默认，直接用这套）**：标签数量固定、与 ViewPager 一一对应、点击即切页、下划线指示条跟随滑动。
...
- **需人工裁定的情况（命中任一条 → 暂停交用户裁定，不要自行决定）**：
  ...
  3. **无 ViewPager / 手动联动**：标签只切换局部视图、不接 VP，需要用 `FragmentContainerHelper` 手动 `handlePageSelected`。
