...
test('creates a complete local OpenSpec change scaffold', async () => {
  ...
  try {
    ...
    assert.equal(change.path, path.join(root, 'cwfspec', 'changes', 'add-auth'));
    assert.equal(
      await readFile(change.metadataPath, 'utf8'),
      `schema: "custom-schema"\ncreated: ${new Date().toISOString().slice(0, 10)}\ngoal: "Add account authentication"\n`
    );
    assert.equal(await readFile(path.join(root, 'cwfspec', 'config.yaml'), 'utf8'), 'schema: spec-driven\n');
...
test('does not search an ancestor for an OpenSpec root', async () => {
  ...
  try {
    ...
    assert.equal(change.path, path.join(nested, 'cwfspec', 'changes', 'fix-login'));
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test('the cwf executable returns machine-readable change details in its configured home', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-openspec-'));
  try {
    const { stdout } = await execFileAsync(process.execPath, [
      path.join(process.cwd(), 'bin', 'cwf.js'),
      'new',
      'change',
      'add-billing',
      '--json',
      ...
      cwd: root,
      env: { ...process.env, CWF_HOME: root },
    });
    const output = JSON.parse(stdout);
    assert.equal(output.change.id, 'add-billing');
    assert.equal(
      output.change.path,
      path.join(path.resolve(root), 'cwfspec', 'changes', 'add-billing')
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
