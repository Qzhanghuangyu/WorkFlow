...
test('installs the shared skill template for Claude and Codex', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
  try {
    ...
    const preflightMetadata = await readFile(
      path.join(projectPath, '.codex', 'skills', 'cwf-preflight', 'agents', 'openai.yaml'),
      'utf8'
    );
    assert.match(preflightMetadata, /display_name: "CWF Preflight"/);
  } finally {
...
test('installs only the tools selected during setup', async () => {
  ...
  try {
    ...
    const templates = await getSkillTemplates();
    assert.equal(
      await readFile(path.join(projectPath, '.claude', 'skills', 'cwf-propose', 'SKILL.md'), 'utf8'),
      templates.find((template) => template.name === 'cwf-propose').content
    );
...
test('preflight creates the change record and propose only requires that change', async () => {
  ...
  assert.doesNotMatch(propose.content, /cwf new change "<name>"/);
});

test('installs spec constraint docs into .customworkflow/spec for any tool', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
  try {
    await installSkills(projectPath, ['claude']);
    const specs = await getSpecTemplates();
...
test('copies existing OpenSpec project specs into .customworkflow/spec recursively', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
  try {
...
test('installs the Claude PreToolUse hook and settings entry', async () => {
  ...
  try {
    ...
    assert.match(skillHook.hooks[0].command, /cwf-spec-guard\.mjs/);
  } finally {
    await rm(projectPath, { recursive: true, force: true });
  }
});
...
test('does not install the Claude hook when only Codex is selected', async () => {
  ...
  try {
    ...
    assert.match(agents, /\.customworkflow\/spec/);
  } finally {
    await rm(projectPath, { recursive: true, force: true });
  }
});
...
test('hook injects spec context for a cwf skill', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
  ...
  try {
    ...
    await rm(projectPath, { recursive: true, force: true });
    await rm(path.join(os.tmpdir(), 'cwf-spec-guard', `${sessionId}.json`), { force: true });
  }
});

test('hook injects global and analysis constraints for cwf-preflight', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
...
test('hook does not inject for a non-cwf skill', async () => {
  const projectPath = await mkdtemp(path.join(os.tmpdir(), 'customworkflow-'));
  const sessionId = `noncwf-${path.basename(projectPath)}`;
  try {
    await installSkills(projectPath, ['claude']);
    const { stdout } = await runHook(projectPath, {
      session_id: sessionId,
...
test('hook deduplicates spec injection within a session', async () => {
  ...
  try {
    ...
    const first = JSON.parse(
      (await runHook(projectPath, { session_id: sessionId, tool_input: { name: 'cwf-propose' } }))
        .stdout
    ...
    const third = JSON.parse(
      (
        await runHook(projectPath, {
          session_id: sessionId,
          tool_input: { name: 'cwf-apply-change' },
        })
      ).stdout
    ).hookSpecificOutput.additionalContext;
...
test('hook re-injects for a different session', async () => {
  ...
  try {
    await installSkills(projectPath, ['claude']);
    await runHook(projectPath, { session_id: sessA, tool_input: { name: 'cwf-propose' } });
    const other = JSON.parse(
      (await runHook(projectPath, { session_id: sessB, tool_input: { name: 'cwf-propose' } }))
        .stdout
    ).hookSpecificOutput.additionalContext;
    ...
    for (const sid of [sessA, sessB]) {
      await rm(path.join(os.tmpdir(), 'cwf-spec-guard', `${sid}.json`), { force: true });
    }
  }
});
...
test('installs the Codex SessionStart hook and config registration', async () => {
  ...
  try {
    ...
    assert.match(config, /cwf-spec-session\.mjs/);
...
test('Codex SessionStart hook injects soul.md and phase pointers', async () => {
  ...
  try {
    ...
    assert.match(ctx, /CustomWorkFlow/);
    // Phase pointers are present.
    assert.match(ctx, /cwf-propose/);
    assert.match(ctx, /\[架构必读\]propose\.md/);
  } finally {
    await rm(projectPath, { recursive: true, force: true });
  }
